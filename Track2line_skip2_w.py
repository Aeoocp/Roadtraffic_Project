  ...
  while True:
    ...
    for track in tracker.tracks:
      ...
      if track_cls == "car":
        ...
        cv2.putText(frame, "ID: " + str(track.track_id), (int(bbox[0]), int(bbox[1])), 0, 1.5e-3 * frame.shape[0], (255, 0, 0), 2)
    ...
    # Draw total count.
    cv2.putText(frame, "Total: {}".format(str(total_counter)), (int(0.7 * frame.shape[1]), int(0.1 * frame.shape[0])), 0,
    ...
    for cls in class_counter:
      ...
      cv2.putText(frame, str(cls) + " " + str(class_count), (int(0.7 * frame.shape[1]), int(y)), 0,
                  1.5e-3 * frame.shape[0], (255, 255, 255), 2)
    ...
    cv2.putText(frame, "frame_index " + str(frame_index), (int(0.75 * frame.shape[1]), int(0.9 * frame.shape[0])), 0,
                  1.5e-3 * frame.shape[0], (255, 255, 255), 2)
